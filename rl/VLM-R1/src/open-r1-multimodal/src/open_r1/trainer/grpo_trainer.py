...
class RepeatRandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        indexes = [indexes[i: i + self.batch_size]
...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert isinstance(
            model, str), "model must be a string in the current implementation"
        model_id = model
        ...
        model_cls = self.vlm_module.get_model_class(
        ...
        if processing_class is None:
            ...
            processing_class = processing_cls.from_pretrained(
                model_id, trust_remote_code=model_init_kwargs.get("trust_remote_code", None))
            for processing_keyword in self.vlm_module.get_custom_processing_keywords():
                if processing_keyword in kwargs:
                    setattr(processing_class, processing_keyword,
            ...
            if getattr(processing_class, "tokenizer",  None) is not None:
                ...
                assert isinstance(
                    processing_class, PreTrainedTokenizerBase), "processing_class must be an instance of PreTrainedTokenizerBase if it has no tokenizer attribute"
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        prompts_text = self.vlm_module.prepare_prompt(
            self.processing_class, inputs)
        ...
        for x in inputs:
            if "image" in x:
                ...
                imgs = [PIL.Image.open(
                    p) for p in self._get_key_from_inputs(x, "image_path")]
                ...
                try:
                    ...
                    if w < 28 or h < 28:
                        ...
                        if w < h:
                            ...
                            new_w = int(w * (28/h))
                    img = img.resize(
                        (new_w, new_h), PIL.Image.Resampling.LANCZOS)
                except:
                    pass
                images.append(img)
        ...
        attention_mask = torch.cat(
            [prompt_mask, completion_mask], dim=1)  # (B, P+C)

        # Get the multimodal inputs
        multimodal_keywords = self.vlm_module.get_custom_multimodal_keywords()
        multimodal_inputs = {
            k: prompt_inputs[k] if k in prompt_inputs else None for k in multimodal_keywords}
        with torch.no_grad():
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.state.global_step % self.num_iterations == 0:
            ...
            self._buffered_inputs[self._step %
        ...
        clip_ratio = (is_clipped * completion_mask).sum() / \
            completion_mask.sum()
